...
* Install a nginx ingress controller using helm
    ...
    `helm install stable/nginx-ingress --name promethus-nginx`

* We will deploy Promethus components in the monitoring namespace. Due to a bug in helm, it is possible for the 4 CRDs that are created by this chart to fail to get fully deployed before Helm attempts to create resources that require them. Create CRDs. Wait for CRDs to be created, which should only take a few seconds
...
NAME                                                     READY   STATUS    RESTARTS   AGE
alertmanager-prometheus-prometheus-oper-alertmanager-0   2/2     Running   0          112m
prometheus-grafana-65bbfddc6d-c6k6z                      2/2     Running   0          112m
prometheus-kube-state-metrics-6fc57fc485-dlsmf           1/1     Running   0          112m
prometheus-prometheus-node-exporter-79qb2                1/1     Running   0          112m
prometheus-prometheus-node-exporter-d727c                1/1     Running   0          112m
prometheus-prometheus-node-exporter-z8xp5                1/1     Running   0          112m
prometheus-prometheus-oper-operator-74dd48f994-n5tc7     1/1     Running   0          112m
prometheus-prometheus-prometheus-oper-prometheus-0       3/3     Running   1          111m
```
...
* For the Grafana dashboard access, the ingress controller is set up to route based on the incoming URL. To reach the correct Grafana endpoint, we must use the Fully Qualified Domain Name (FQDN) and file path to the ingress controller. In our example, FQDN (grafana.test.example.com) must be resolvable to an IP.  To find the external IP address of the Grafana service
    ...
    `kubectl get svc | grep  promethus-nginx-nginx-ingress-controller | awk '{print $4}' | awk -F , '{print $1}'`
    ...
    NAME                                             TYPE           CLUSTER-IP       EXTERNAL-IP                  PORT(S)                      AGE
    kubernetes                                       ClusterIP      10.100.200.1     <none>                       443/TCP                      2d23h
    prometheus-nginx-nginx-ingress-controller        LoadBalancer   10.100.200.192   10.173.62.120,100.64.80.33   80:31650/TCP,443:30572/TCP   119m
    prometheus-nginx-nginx-ingress-default-backend   ClusterIP      10.100.200.226   <none>                       80/TCP                       119m
...
* To access Grafana, enter `https://grafana.test.example.com/` in a web browser -  Username/Password: admin/VMware1!
  ...
  **Note**:  Edit the `etc/hosts` file of the local computer putting the IP and DNS name for the Grafana dashboard to appear in a browser.
